git config --global user.name 'DiegoMuneraT'
git config --global user.email 'damunerat@eafit.edu.co'
...
git remote set-url origin https://x-access-token:${{ secrets.GH_TOKEN }}@github.com/DiegoMuneraT/MLOps-Lab-2.git
git add .
set +e 
